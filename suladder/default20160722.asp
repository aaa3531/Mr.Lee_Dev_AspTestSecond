...
<%
 ...
 ' response.End
  ...
  'response.write Session("point_total")
    ...
    strSQL = "p_sh_game_ladder_reset '" & Session("member_no") & "'"
    'response.Write strSQL
    'response.end
...
<input type="hidden" id="ladderrate"  value="0.0" />
 ...
 <!--
    ...
    function resultMsgHide() {
        document.getElementById("prev_result_box").style.display = "none";
			...
			<!--[e] 게임 종료 이미지 영역 -->
				...
				<div class="up_sec">
					...
					<p class="sec1"><span id="turn_no1"><%=turn_no1 %> </span><span>베팅가능 시간 :</span><strong id="dtimer"></strong><p>
		...
		<form id="form1" name="formBet" method="post" >
                ...
                <div style="height:15px;"></div>
					...
					<input type="button" class="btn_betting03" value="베팅취소" onclick="javascript:basketDelete(this);"/>
				</div>
				</div>
                </form>
	...
	<!--[e] ASIDE -->		
				...
				<iframe id="listframe" frameborder="0" marginwidth="0" marginheight="0" width="100%" height="530px" name="content" scrolling="no" src="list_frame.asp"  style="padding:0 0 3px 0; "></iframe>
