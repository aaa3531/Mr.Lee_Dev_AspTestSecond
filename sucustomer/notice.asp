...
<%        
  if request("notice_no") <> "" then
  notice_no = request("notice_no")
  end if

  strSQL = "p_sh_notice_list "
...
%>
	...
	<div id="container">
		...
		<div id="content" class="subcon">
			<div class="snb_title">
				<ul>
				<li class="snb_01">공지사항<span>HOME > 고객센터 > 공지사항</span></li>
				</ul>
    ...
    <% 
       ...
       Do While Not rsData.EOF and RowCount > 0    %> 
							<tr>
                            <% if rsData("notice_order") > "0" then %>
								<td>※</td>
                                <% else %>
								<td><%=rsData("notice_no") %></td>
                                <% end if %>
								<td class="board_title"><a href="notice.asp?notice_no=<%=rsData("notice_no") %>"><%=rsData("notice_desc") %></a></td>
								<td><%=rsData("register_date") %></td>
							</tr>
    <% if rsData("notice_no") * 1 - notice_no = 0 then %>
							<tr class="ans" style="display: table-row;">
								<td></td>
								<td colspan="2" >
									<div class="ans_content"><p><%=rsData("notice_note") %></p></div>
								</td>
							</tr>
    <% end if %>
    <% 	
...
<!--#include virtual="/_include/asp_page_function.asp"-->
			<div class="paging">
<%
	Response.Write ShowPageBar("notice.asp", Cus_pageSize, totalRecord, cPage, "/images/btn_board_pre.gif","/images/btn_board_next.gif",Cus_Tar)
%>
			</div>
	<%end if%>		
	<!-- 페이징 처리 끝-->
